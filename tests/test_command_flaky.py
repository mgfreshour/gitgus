...
def test_create_no_options(monkeypatch):
    ...
    monkeypatch.setattr("gitgus.workflows.flaky_wf.FlakyWorkflow._get_job_name", mock_get_job_name)
    mock_get_job_name.return_value = "funhouse"
    mock_get_test_stacktrace = MagicMock()
    monkeypatch.setattr("gitgus.workflows.flaky_wf.FlakyWorkflow._get_test_stacktrace", mock_get_test_stacktrace)
...
def test_report_all_builds(monkeypatch):
    ...
    monkeypatch.setattr("gitgus.workflows.flaky_wf.FlakyWorkflow.build_report", mock_report)
    runner = CliRunner()
    result = runner.invoke(
        flaky_app, ["report-all-builds", "bobo/jobs", "--start-date", "2021-01-01", "--end-date", "2021-01-02"]
    )
    assert result.exit_code == 0, result.stdout
    # Assert inclusive dates
    mock_report.assert_called_once_with("bobo/jobs", datetime(2021, 1, 1, 0, 0), datetime(2021, 1, 2, 23, 59, 59))
    # TODO: assert output
...
def test_report(monkeypatch):
    ...
    monkeypatch.setattr("gitgus.workflows.flaky_wf.FlakyWorkflow.get_flaky_tagged_tests", mock_get_flaky_tagged_tests)
    runner = CliRunner()
    result = runner.invoke(
        flaky_app, ["report-failed-builds", "bobo/jobs", "--start-date", "2021-01-01", "--end-date", "2021-01-02"]
    )
    assert result.exit_code == 0, result.stdout
    # Assert inclusive dates
    mock_report.assert_called_once_with("bobo/jobs", datetime(2021, 1, 1, 0, 0), datetime(2021, 1, 2, 23, 59, 59))
