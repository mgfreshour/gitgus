...
from gitgus.workflows.flaky_wf import (
    FlakyWorkflow,
    TICKET_TYPE_FIX,
    TICKET_TYPE_DISABLE,
)
...
def report_all_builds(
    ...
    all_results_count, job_result_count = flaky.build_report(
...
def report_failed_builds(
    ...
    for case in all_testcases:
        ts = datetime.strptime(
            case["timestamp"], "%d %b %Y %H:%M:%S %Z"
        )  # '19 May 2023 15:56:36 GMT'
        if ts < oldest_build:
            oldest_build = ts
...
def create_tickets(
    build: str = typer.Option(
        default=0, help="Build number test failed in", prompt=True
    ),
    ...
    no_disable_ticket: bool = typer.Option(
        default=False, help="Don't create a disable ticket"
    ),
):
    ...
    print(
        "Remember to update the product tag and add the stack trace to the fix ticket!"
    )
