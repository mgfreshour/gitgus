...
def get_auth_token(
    ...
    client_id,
    scopes: list[str],
    code_getter: Callable[[str, str], None],
):
    device_code, user_code, code_url, expires_in = get_verification_code(
        device_uri, client_id, scopes
    )
...
def get_verification_code(device_uri, client_id, scopes: list[str]):
    ...
    if resp.status_code != 200:
        raise RuntimeError(
            f"Error: no authorization code returned from browser: {resp.text}"
        )
    payload = resp.json()
    return (
        payload["device_code"],
